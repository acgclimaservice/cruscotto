...
from models import (DDTIn, ArticoloIn, DDTOut, ArticoloOut, 
                    ...
                    OffertaFornitore, DettaglioOfferta)
...
def parse_pdf_claude():
    ...
    try:
        ...
        try:
            ...
            # Parse con entrambe le AI per ottenere il risultato migliore
            result = parser.parse_with_both_ai(file)
...
def create_ddt_from_import():
    ...
    try:
        ...
        for art_data in data.get('articoli', []):
            ...
            articolo = ArticoloIn(
                ddt_id=nuovo_ddt.id,
                codice_interno=art_data.get('codice_interno') or art_data.get('codice', ''),
...
def ddt_in_page():
    ...
    try:
        # Ordina: prima bozze, poi confermati per numero in ordine decrescente
        ddts = DDTIn.query.order_by(
            DDTIn.stato.asc(),  # bozza prima di confermato
            DDTIn.numero_ddt.desc()  # numero documento decrescente
        ).all()
...
def pdf_unificato_ddt(id):
    ...
    try:
        ...
        content.append(Paragraph("<b>ACG CLIMA SERVICE S.R.L.</b><br/>Sede Legale: Via Duccio Galimberti 47 - 15121 Alessandria (AL)<br/>Sede Operativa: Via Zanardi Bonfiglio 68 - 27058 Voghera (PV)<br/>Tel: 0383/640606 - Email: info@acgclima.it<br/>P.IVA: 02735970069 - C.F: 02735970069", styles['Normal']))
        content.append(Spacer(1, 0.5*cm))
...
def nuovo_ddt_in():
    ...
    try:
        ...
        if request.form.get('data_ddt_origine'):
            nuovo_ddt.data_ddt_origine = datetime.strptime(
                request.form['data_ddt_origine'], '%Y-%m-%d'
            ).date()
        else:
            nuovo_ddt.data_ddt_origine = datetime.now().date()
    ...
    try:
        ...
        for art in articoli:
            ...
            if True:  # Processa sempre tutti gli articoli
                # Cerca o crea articolo nel catalogo
...
def elimina_ddt_in(id):
    """Elimina DDT IN e tutti gli articoli collegati"""
    try:
        ddt = DDTIn.query.get_or_404(id)
        ...
        for art in articoli_in:
            if art.codice_interno:
                ...
                if catalogo_art:
                    # Se il DDT è confermato, sottrai la quantità dalla giacenza
                    if ddt.stato == 'confermato':
            ...
            # Elimina l'articolo dal DDT IN
            db.session.delete(art)
        
        # Elimina il DDT IN stesso
        ...
        return jsonify({
            'success': True, 
            'message': f'DDT IN {ddt.numero_ddt or f"BOZZA-{id}"} eliminato con successo',
            'articoli_rimossi': len(articoli_in),
            'movimenti_eliminati': len([a for a in articoli_in if a.codice_interno])
        })
            ...
            while f"articoli[{i}][descrizione]" in data:
                if data.get(f"articoli[{i}][descrizione]"):
                    articolo = ArticoloOut(
                        ...
                        costo_unitario=round(float(data.get(f"articoli[{i}][prezzo_unitario]", 0)), 2),  # Arrotondamento a 2 decimali
...
def inventario_page():
    """Pagina inventario - mostra articoli del catalogo"""
    try:
        # Prendi articoli dal catalogo
        articoli = CatalogoArticolo.query.filter_by(attivo=True).all()
...
def report_mastrini():
    ...
    try:
        ...
        spese_query = db.session.execute(db.text("""
            SELECT 
                d.mastrino_ddt as mastrino,
                m.descrizione as descrizione,
                'ACQUISTO' as tipo,
                COUNT(d.id) as numero_ddt,
                SUM(CASE WHEN a.quantita > 0 THEN a.quantita * a.costo_unitario ELSE 0 END) as totale
            FROM ddt_in d
            LEFT JOIN articolo_in a ON d.id = a.ddt_id  
            LEFT JOIN mastrino m ON d.mastrino_ddt = m.codice
            WHERE d.stato = 'confermato' AND d.mastrino_ddt IS NOT NULL AND d.mastrino_ddt != ''
            GROUP BY d.mastrino_ddt, m.descrizione
        ...
        ricavi_query = db.session.execute(db.text("""
            SELECT 
                d.mastrino_ddt as mastrino,
                m.descrizione as descrizione,
                'RICAVO' as tipo, 
                ...
                SUM(CASE WHEN a.quantita > 0 THEN a.quantita * a.costo_unitario ELSE 0 END) as totale
            FROM ddt_out d
            LEFT JOIN articolo_out a ON d.id = a.ddt_id
            LEFT JOIN mastrino m ON d.mastrino_ddt = m.codice
            WHERE d.stato = 'confermato' AND d.mastrino_ddt IS NOT NULL AND d.mastrino_ddt != ''
            GROUP BY d.mastrino_ddt, m.descrizione
            ORDER BY totale DESC
        ...
        totale_spese = sum(m['totale'] for m in spese_mastrini if m['totale'])
        totale_ricavi = sum(m['totale'] for m in ricavi_mastrini if m['totale'])
...
def export_report_mastrini_excel():
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            df_spese.to_excel(writer, sheet_name='Report Mastrini', index=False)
...
def aggiorna_inventario():
    ...
    try:
        ...
        for art in articoli_ddt_in:
            ...
            chiave = f"{codice_interno}|{art.descrizione}"
        ...
        for chiave, dati in inventario_aggiornato.items():
            ...
            # Cerca articolo esistente prima per codice interno generato
            articolo = CatalogoArticolo.query.filter_by(codice_interno=codice).first()
            
            # Se non trovato per codice generato, cerca per codice fornitore (per aggiornare codici esistenti)
            if not articolo and dati['codice_fornitore']:
                articolo = CatalogoArticolo.query.filter_by(codice_interno=dati['codice_fornitore']).first()
            
            # Se non trovato per codice, cerca per descrizione (per articoli senza codice)
            if not articolo and not codice.startswith('ART_SENZA_CODICE'):
                articolo = CatalogoArticolo.query.filter_by(descrizione=descrizione).first()
...
def nuovo_preventivo():
    ...
    try:
        ...
        cliente_id_value = request.form.get('cliente_id')
        cliente_id = int(cliente_id_value) if cliente_id_value else None
...
def modifica_preventivo(id):
    ...
    try:
        ...
        preventivo.iva = float(request.form.get('iva', 22))
    ...
    try:
        ...
        for dettaglio in dettagli_preventivo:
            articolo_out = ArticoloOut(
                ...
                costo_unitario=round(float(dettaglio.prezzo_unitario or 0), 2)  # Arrotondamento a 2 decimali
            )
...
def stampa_ordine_pdf(id):
    ...
    content.append(Paragraph("Sede Legale: Via Duccio Galimberti 47 - 15121 Alessandria (AL)<br/>Sede Operativa: Via Zanardi Bonfiglio 68 - 27058 Voghera (PV)<br/>Tel: 0383/640606 - Email: info@acgclima.it<br/>P.IVA: 02735970069 - C.F: 02735970069", 
...
def offerte_pdf(id):
    ...
    try:
        ...
        # Informazioni azienda
        content.append(Paragraph("<b>ACG CLIMA SERVICE S.R.L.</b>", ParagraphStyle('CompanyInfo', parent=normal_style, alignment=TA_CENTER)))
        content.append(Paragraph("Via della Tecnica, 123 - 00100 Roma<br/>Tel: +39 06 1234567 - Email: info@acgclima.it<br/>P.IVA: IT12345678901", 
                                ParagraphStyle('CompanyDetails', parent=normal_style, alignment=TA_CENTER, fontSize=9)))
        content.append(Spacer(1, 0.8*cm))
...
@app.route('/impostazioni/mastrino/nuovo', methods=['POST'])
def nuovo_mastrino_manuale():
    """Crea nuovo mastrino manualmente dalle impostazioni"""
    try:
        data = request.get_json()
        if not data:
        ...
        if tipo not in ['ACQUISTI', 'RICAVI']:
        ...
        return jsonify({
            ...
            'mastrino_id': nuovo_mastrino.id
        })
        
    except Exception as e:
        db.session.rollback()
        print(f"Errore creazione mastrino: {e}")
        return jsonify({'success': False, 'error': f'Errore durante creazione: {str(e)}'}), 500
...
def nuovo_collegamento_mastrini():
    ...
    try:
        ...
        db.session.execute(db.text("""
            INSERT INTO collegamento_mastrini (mastrino_acquisto_id, mastrino_ricavo_id, descrizione_collegamento)
            VALUES (:acq, :ric, :desc)
        """), {
            'acq': mastrino_acquisto_id,
            'ric': mastrino_ricavo_id, 
            'desc': descrizione
        })
    ...
    try:
        ...
        movimenti_query = """
            ...
            SELECT 
                ...
                ao.descrizione,
                ao.quantita,
                ao.costo_unitario as valore_unitario,
                do.data_ddt as data_movimento,
    ...
    try:
        ...
        fornitori_query = """
            SELECT 
                di.fornitore, 
                COUNT(*) as totale_ddt, 
                MAX(di.data_ddt) as ultimo_ddt,
                CAST(julianday('now') - julianday(MAX(di.data_ddt)) AS INTEGER) as giorni_ultimo_ddt,
                COALESCE(SUM(ai.quantita * ai.costo_unitario), 0.0) as valore_totale
            FROM ddt_in di
            LEFT JOIN articolo_in ai ON di.id = ai.ddt_id
            WHERE di.fornitore != '' AND di.fornitore IS NOT NULL AND di.stato = 'confermato'
...
def report_articoli():
    ...
    try:
        ...
        ).group_by(ArticoloIn.codice_interno).subquery()
        
        articoli_out = db.session.query(
            ArticoloOut.codice_interno,
            func.sum(ArticoloOut.quantita).label('qty_out')
        ).group_by(ArticoloOut.codice_interno).subquery()
    ...
    try:
        ...
        clienti_query = """
            SELECT 
                do.destinazione, 
                COUNT(*) as totale_ddt, 
                MAX(do.data_ddt) as ultimo_ddt,
                CAST(julianday('now') - julianday(MAX(do.data_ddt)) AS INTEGER) as giorni_ultimo_ddt,
                COALESCE(SUM(ao.quantita * ao.costo_unitario), 0.0) as valore_totale
            FROM ddt_out do
            LEFT JOIN articolo_out ao ON do.id = ao.ddt_id
            WHERE do.destinazione != '' AND do.destinazione IS NOT NULL AND do.stato = 'confermato'
...
def report_commesse():
    ...
    try:
        ...
        tipologie_query = """
            ...
            FROM commessa 
            WHERE tipologia != '' AND tipologia IS NOT NULL
            GROUP BY tipologia 
        ...
        clienti_query = """
            ...
            LEFT JOIN (
                SELECT destinazione, COUNT(*) as totale_ddt_out, 
                       SUM(ao.quantita * ao.costo_unitario) as valore_totale_out
                FROM ddt_out do
                LEFT JOIN articolo_out ao ON do.id = ao.ddt_id
                WHERE do.stato = 'confermato'
            ...
            WHERE c.cliente_nome != '' AND c.cliente_nome IS NOT NULL
        ...
        totale_commesse = db.session.execute(text("SELECT COUNT(*) FROM commessa")).scalar()
        commesse_aperte = db.session.execute(text("SELECT COUNT(*) FROM commessa WHERE stato = 'aperta'")).scalar()
        commesse_chiuse = db.session.execute(text("SELECT COUNT(*) FROM commessa WHERE stato = 'chiusa'")).scalar()
...
def export_report(report_type):
    ...
    try:
        ...
        if report_type == 'movimenti':
            ...
            clienti_query = """
                SELECT nome_origine, COUNT(*) as totale_ddt, MAX(data_ddt) as ultimo_ddt,
                       CAST(julianday('now') - julianday(MAX(data_ddt)) AS INTEGER) as giorni_ultimo_ddt
                FROM ddt_out 
                WHERE nome_origine != '' AND nome_origine IS NOT NULL
            ...
            fornitori_query = """
                SELECT ddt_in.fornitore, COUNT(*) as totale_ddt, 
                       MAX(ddt_in.data_ddt) as ultimo_ddt,
                       CAST(julianday('now') - julianday(MAX(ddt_in.data_ddt)) AS INTEGER) as giorni_ultimo_ddt,
                ...
                WHERE ddt_in.fornitore != '' AND ddt_in.fornitore IS NOT NULL
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("SISTEMA GESTIONE DDT - VERSIONE 2.5")
    print("=" * 50)
