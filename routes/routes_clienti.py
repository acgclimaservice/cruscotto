        ...
        try:
            ...
            if not ragione_sociale:
                if request.is_json:
                    ...
                    return redirect(url_for('clienti.nuovo_cliente'))
            ...
            if partita_iva:
                ...
                if esistente:
                    if request.is_json:
                        ...
                        flash(f'Cliente con P.IVA {partita_iva} già esistente', 'error')
                        return redirect(url_for('clienti.nuovo_cliente'))
            ...
            nuovo_cliente = Cliente(
                ...
                pec=get_data('pec'),
                codice_sdi=get_data('codice_sdi'),
...
def elimina_cliente(id):
    ...
    try:
        ...
        # Verifica se il cliente è utilizzato in DDT OUT
        from models import DDTOut
        ddt_collegati = DDTOut.query.filter_by(destinazione=cliente.ragione_sociale).count()
        
        if ddt_collegati > 0:
            return jsonify({
                'success': False,
                'error': f'Impossibile eliminare. Il cliente è utilizzato in {ddt_collegati} DDT OUT'
            })
