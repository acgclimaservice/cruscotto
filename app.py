...
def pdf_unificato_ddt(id):
    ...
    try:
        ...
        # Genera PDF usando WeasyPrint + unione con PDF originale
        import base64
        from io import BytesIO
        from PyPDF2 import PdfMerger
        import weasyprint
        
        print(f"[PDF_UNIFICATO] Generando PDF con WeasyPrint...")
        
        # Genera DDT come HTML per WeasyPrint (senza PDF embedded)
        html_ddt = render_template('pdf/ddt-in-pdf-simple.html', ddt=ddt, articoli=articoli)
        
        # Converte HTML in PDF usando WeasyPrint
        ddt_pdf_bytes = weasyprint.HTML(string=html_ddt, base_url=request.url_root).write_pdf()
        print(f"[PDF_UNIFICATO] DDT PDF generato: {len(ddt_pdf_bytes)} bytes")
        
        # Decodifica PDF allegato
        pdf_allegato_bytes = base64.b64decode(ddt.pdf_allegato)
        print(f"[PDF_UNIFICATO] PDF allegato: {len(pdf_allegato_bytes)} bytes")
        
        # Unisce i due PDF usando PyPDF2
        merger = PdfMerger()
        
        # Aggiungi DDT generato dal sistema
        ddt_stream = BytesIO(ddt_pdf_bytes)
        merger.append(ddt_stream)
        
        # Aggiungi PDF originale fornitore
        pdf_stream = BytesIO(pdf_allegato_bytes)
        ...
        print(f"[PDF_UNIFICATO] PDF unificato creato: {len(output_stream.getvalue())} bytes")
...
# All orphaned ReportLab code completely removed - app should now start correctly
...
def nuova_offerta():
    ...
    if request.method == 'POST':
        try:
            ...
            # Aggiungi articoli se presenti - versione semplificata e sicura
            import re
            dettagli_dict = {}
            
            # Estrai dati articoli dal form data
            ...
            # Salva articoli
            for index in sorted(dettagli_dict.keys()):
                art_data = dettagli_dict[index]
                ...
                try:
                    articolo = DettaglioOfferta(
                        ...
                        unita_misura=art_data.get('unita', 'PZ'),
                        ...
                        tempo_consegna=art_data.get('consegna', ''),
                        note=art_data.get('note', '')
                    )
                    db.session.add(articolo)
                except Exception as e:
                    print(f"Errore salvataggio articolo {index}: {e}")
                    continue
...
def stampa_mpls(id):
    ...
    try:
        ...
        <head>
            <meta charset="UTF-8">
            <title>MPLS {mpls.numero_mpls}</title>
            <style>
        ...
        <body>
            ...
            <div class="info-section">
                <div class="info-grid">
                    <div>
                        <div class="info-item">
                            <span class="info-label">Cliente:</span> {mpls.cliente_nome}
                        </div>
                        <div class="info-item">
                            <span class="info-label">Data Creazione:</span> {mpls.data_creazione.strftime('%d/%m/%Y') if mpls.data_creazione else '-'}
                        </div>
                        <div class="info-item">
                            <span class="info-label">Stato:</span> {mpls.stato.upper()}
                        </div>
                    </div>
                    <div>
                        <div class="info-item">
            ...
            <div class="totali">
                <div class="info-grid">
                    <div>
                        ...
                        <div class="info-item">
                            <span class="info-label">Ore Manodopera:</span> {mpls.ore_manodopera or 0} ore
                        </div>
                        <div class="info-item">
                    ...
                    <div>
                        <div class="info-item">
                            <span class="info-label">Sovrapprezzo:</span> € {mpls.sovrapprezzo or 0:.2f}
                        </div>
                        <div class="info-item">
