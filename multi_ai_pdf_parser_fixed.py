...
IMPORTANTE: quantità come numero puro (2 non 2,000), prezzi come decimali."""
    ...
    def parse_ordine_pdf(self, file_path, ai_service='claude'):
        """Parse PDF di offerta fornitore con fallback automatico"""
        print(f"PARSING ORDINE AVVIATO: AI preferita = {ai_service.upper()}")
        
        # Prima prova con AI preferita
        if ai_service == 'claude':
            result = self.parse_ordine_with_claude(file_path)
            if result['success']:
                print("Parsing ordine Claude completato")
                return result
            
            print(f"Claude fallito: {result.get('error')}")
            print("Tentativo fallback a Gemini per ordine...")
            result = self.parse_ordine_with_gemini(file_path)
            if result['success']:
                print("Parsing ordine Gemini completato (fallback)")
                if 'data' in result:
                    result['data']['ai_reason'] = f"Gemini AI usato come fallback (Claude fallito: {result.get('error', 'errore sconosciuto')[:50]})"
                return result
            ...
            result = self.parse_ordine_with_gemini(file_path)
            if result['success']:
                print("Parsing ordine Gemini completato")
                return result
                
            print(f"Gemini fallito: {result.get('error')}")
            print("Tentativo fallback a Claude per ordine...")
            result = self.parse_ordine_with_claude(file_path)
            if result['success']:
                print("Parsing ordine Claude completato (fallback)")
                if 'data' in result:
                    result['data']['ai_reason'] = f"Claude AI usato come fallback (Gemini fallito: {result.get('error', 'errore sconosciuto')[:50]})"
                return result
        
        return {'success': False, 'error': 'Entrambe le AI hanno fallito per parsing ordine'}
