...
class Autocomplete {
    ...
    init() {
        ...
        document.addEventListener('click', (e) => {
            const containerClass = this.input.name.replace(/\[|\]|\./g, '\\$&');
            if (!e.target.closest(`.autocomplete-container-${containerClass}`)) {
    ...
    adjustPosition() {
        ...
        const listHeight = 200; // max-height della lista
        const viewportHeight = window.innerHeight;
        const spaceBelow = viewportHeight - inputRect.bottom;
        
        // Se non c'è spazio sotto e c'è spazio sopra, posiziona sopra
        if (spaceBelow < listHeight && inputRect.top > listHeight) {
            this.suggestionsList.style.top = 'auto';
            this.suggestionsList.style.bottom = '100%';
            this.suggestionsList.style.borderTop = '1px solid #ccc';
            this.suggestionsList.style.borderBottom = 'none';
            this.suggestionsList.style.borderRadius = '4px 4px 0 0';
        } else {
            // Posizione normale (sotto)
            this.suggestionsList.style.top = '100%';
            this.suggestionsList.style.bottom = 'auto';
            this.suggestionsList.style.borderTop = 'none';
            this.suggestionsList.style.borderBottom = '1px solid #ccc';
...
window.AutocompleteHelpers = {
    ...
    // Codici Interni per ricerca rapida
    setupCodiciInterniAutocomplete: function(inputElement, options = {}) {
        return new Autocomplete(inputElement, {
            endpoint: '/api/articoli/search',
            placeholder: 'Digita codice interno...',
            displayKey: 'codice_interno',
            template: (item, query) => {
                const codiceInterno = window.AutocompleteHelpers.highlightMatch(item.codice_interno || '', query);
                const descrizione = item.descrizione || '';
                const fornitore = item.fornitore_principale || '';
                
                return `<strong>${codiceInterno}</strong><br><small style="color: #666;">${descrizione}${fornitore ? ' - ' + fornitore : ''}</small>`;
            },
            onSelect: (item, input) => {
                input.value = item.codice_interno;
                
                // Trova la riga corrente
                const riga = input.closest('.dettaglio-riga, .dettaglio-row');
                if (riga) {
                    // Auto-compila anche descrizione e codice fornitore se disponibili
                    const descrizioneField = riga.querySelector('input[name*="[descrizione]"]');
                    const codiceFornitoreField = riga.querySelector('input[name*="[codice_fornitore]"]');
                    
                    if (descrizioneField && item.descrizione) {
                        descrizioneField.value = item.descrizione;
                    }
                    if (codiceFornitoreField && item.codice_fornitore) {
                        codiceFornitoreField.value = item.codice_fornitore;
                    }
